...
class PDFReportGenerator {
  constructor() {
    ...
    this.height = 841.89;
    this.margin = 50;
    this.chartJSNodeCanvas = new ChartJSNodeCanvas({ width: 400, height: 300 });
  }

  async generateDailyReport() {
    try {
      console.log('📊 Generando informe diario de daños...');
      ...
      const fileName = `informe_danos_${new Date().toISOString().split('T')[0]}.pdf`;
      ...
      // Generar contenido del informe con las secciones específicas solicitadas
  ...
  async generateHeader() {
    ...
    this.doc
      .fontSize(24)
      .font('Helvetica-Bold')
      .text('INFORME DIARIO DE DAÑOS 2025', { align: 'center' })
      .moveDown(0.5);
  ...
  async generateGraficas() {
    this.doc
      .fontSize(18)
      .font('Helvetica-Bold')
      .text('📈 GRÁFICAS Y VISUALIZACIONES', { underline: true })
      .moveDown(1);

    try {
      // Gráfica 1: Evolución de daños acumulados
      await this.generateEvolucionDanosChart();
      this.doc.moveDown(1);

      // Gráfica 2: Distribución Hembra vs Macho
      await this.generateDistribucionHembraMachoChart();
      this.doc.moveDown(1);

      // Gráfica 3: Top operadores
      await this.generateTopOperadoresChart();
      this.doc.moveDown(1);

      // Gráfica 4: Superficie por mes
      await this.generateSuperficieMensualChart();
      ...
      this.doc
        .fontSize(12)
        .font('Helvetica')
        .text('⚠️ No se pudieron generar las gráficas debido a un error técnico.')
        .moveDown(1);
    }
  }

  async generateEvolucionDanosChart() {
    try {
      const danosData = await this.getDanosAcumuladosData();
      ...
      if (danosData.evolucionMensual && danosData.evolucionMensual.length > 0) {
        this.doc
          .fontSize(14)
          .font('Helvetica-Bold')
          .text('📊 Evolución de Daños Acumulados 2025', { underline: true })
          .moveDown(0.5);
        ...
        const chartConfig = {
          type: 'line',
          data: {
            labels: danosData.evolucionMensual.map(item => item.mes),
            datasets: [
              {
                label: 'Daños Reales',
                data: danosData.evolucionMensual.map(item => item.danosAcumulados),
                borderColor: '#dc3545',
              ...
              },
              {
                ...
                data: danosData.evolucionMensual.map(item => item.metaAcumulada),
                borderColor: '#007bff',
              ...
              {
                label: 'Año Anterior',
                data: danosData.evolucionMensual.map(item => item.anoAnterior),
                borderColor: '#ffc107',
          ...
          options: {
            ...
            plugins: {
              title: {
                display: true,
                text: 'Evolución Mensual de Daños'
              },
              legend: {
                position: 'top'
              }
        ...
        this.doc.image(chartBuffer, this.margin, this.doc.y, { width: 400 });
        this.doc.moveDown(8);
      }
    } catch (error) {
      console.error('Error generando gráfica de evolución:', error);
    }
  }

  async generateDistribucionHembraMachoChart() {
    try {
      const resumenData = await this.getResumenAnualHembraMachoData();
      
      if (resumenData.hembra > 0 || resumenData.macho > 0) {
        this.doc
          .fontSize(14)
          .font('Helvetica-Bold')
          .text('📊 Distribución Hembra vs Macho', { underline: true })
          .moveDown(0.5);
        ...
        const chartConfig = {
          ...
          data: {
            labels: ['HEMBRA', 'MACHO'],
            datasets: [{
              data: [resumenData.hembra, resumenData.macho],
              backgroundColor: [
                'rgba(255, 99, 132, 0.8)',
                'rgba(54, 162, 235, 0.8)'
              ],
        ...
        const chartBuffer = await this.chartJSNodeCanvas.renderToBuffer(chartConfig);
        this.doc.image(chartBuffer, this.margin, this.doc.y, { width: 300 });
        this.doc.moveDown(6);
      }
    } catch (error) {
      console.error('Error generando gráfica de distribución:', error);
    }
  }

  async generateTopOperadoresChart() {
    try {
      const operadoresData = await this.getDetallePorOperadorData();
      ...
      if (operadoresData && operadoresData.length > 0) {
        this.doc
          .fontSize(14)
          .font('Helvetica-Bold')
          .text('👥 Top 10 Operadores por Rendimiento', { underline: true })
          .moveDown(0.5);

        // Tomar solo los top 10
        const top10 = operadoresData.slice(0, 10);
        
        // Crear gráfica de barras
        const chartConfig = {
          type: 'bar',
          data: {
            labels: top10.map(op => op.nombreOperador?.substring(0, 15) || 'N/A'),
            datasets: [{
              label: 'Total Anual',
              data: top10.map(op => op.totalAnual),
      ...
      console.error('Error generando gráfica de operadores:', error);
    }
  }

  async generateSuperficieMensualChart() {
    try {
      const surfaceData = await this.getMetroSuperficieData();
      ...
      if (surfaceData.porMes && surfaceData.porMes.length > 0) {
        this.doc
          .fontSize(14)
          .font('Helvetica-Bold')
          .text('📏 Superficie Limpiada por Mes', { underline: true })
          .moveDown(0.5);

        // Crear gráfica de barras para superficie
        const chartConfig = {
          type: 'bar',
          data: {
            labels: surfaceData.porMes.map(item => item.mes),
            datasets: [{
              label: 'Superficie (m²)',
              data: surfaceData.porMes.map(item => item.superficie),
          ...
          options: {
            ...
            plugins: {
              title: {
                display: true,
                text: 'Superficie Limpiada Mensual'
              },
              legend: {
                display: false
              }
      ...
      console.error('Error generando gráfica de superficie:', error);
    }
  }
  ...
  async generateFooter() {
    this.doc
      .fontSize(10)
      .font('Helvetica')
      .text('Sistema de Reportes Automáticos - Generado automáticamente', { align: 'center' })
      .moveDown(0.5)
